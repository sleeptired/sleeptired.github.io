...
```cpp
// 매 프레임마다 상태를 체크하고 로직을 실행합니다.
void AMyCharacter::Tick(float DeltaTime)
{
    Super::Tick(DeltaTime);

    // 2. [전환 (Transition) & 조건 (Condition)] 검사
    switch (CurrentState)
    {
        ...
        case ECharacterState::Attack:
            // 공격 애니메이션이 끝나면 다시 Idle로 돌아가는 로직 수행
            break;

        case ECharacterState::Dead:
            // 사망 상태에서는 아무것도 하지 않음 (로직 차단)
            break;
    }
}

// 3. [이벤트 (Event)]에 의한 상태 전환
...
// 3. [이벤트 (Event)]에 의한 상태 전환 2 : 외부 시스템(피격)
