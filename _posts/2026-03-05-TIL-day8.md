...
2. **언리얼 엔진 메모리관리**  
   ...
   **언리얼 엔진 메모리 관리 객체 분류**  
   ...
   루트셋에 들어가는 객체와 가비지 컬렉션에 들어가는 객체 차이점: 객체가 하나라도 참조가 되어있는지 아닌지로 판단한다.  

   가비지 컬렉션에 들어가지 않는 종류  
   ex) 루트셋, 참조된 객체  
   
   가비지 컬렉션에 들어가는 종류  
   ex) Destroy된 액터, 참조를 잃은 UObject, 함수 안의 지역 변수, 생성만 하고 어디에도 등록 안 한 UI  
   
   참조는 UPROPERTY()통해 알 수 있다.  
   ...
   **가비지 컬렉션은 단순히 지운다,안 지운다를 넘어, 각 객체의 상태를 플래그(Flag)라는 꼬리표로 관리**
   ...
   RF_BeginDestroyed & RF_FinishDestroyed (시스템이 내부적으로 관리하는 상태 값)
   RF_BeginDestroyed:파괴 로직을 막 시작한 단계, BeginDestroyed() 가상 함수가 호출되었음을 의미
   이 상태의 객체는 아직 메모리에 데이터는 남아있지만, 더 이상 정상적인 기능을 수행할 수 없습니다.
   
   RF_FinishDestroyed: 모든 파괴 준비 작업이 끝난 단계입니다. inishDestroyed() 가상 함수가 호출되었음을 의미
   객체가 차지하던 공간을 비우기 직전의 마지막 단계입니다. 이 플래그가 찍히면 더 이상 엔진의 그 어떤 시스템도 이 객체에 접근해서는 안 됨

   **리플렉션 시스템** -> 엔진툴이랑 호환할려고 사용하는 함수 및 정의 (그냥 우리가 쓰던 c++로 하면 언리얼 엔진에서 못받아들이고 사용을 못함)
