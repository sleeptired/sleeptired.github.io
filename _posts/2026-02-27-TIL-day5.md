...
3. <span style="font-size: 20px">**클래스**</span>  
   ...
   이니셜라이저: 객체가 메모리에 **생성되는 순간**에 값을 채워 넣습니다.  
   대입: 객체가 메모리에 이미 생성된 후(기본값으로), 그 안에 있던 값을 지우고 **새 값**으로 덮어씁니다.  
   ★ 이니셜라이저가 성능에 약간의 이점이 있다.

5. <span style="font-size: 20px">**inline 함수**</span>  
